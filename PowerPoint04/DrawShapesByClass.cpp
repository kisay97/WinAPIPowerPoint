...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN: {
		if (canDrawing) {
			...
			POINT _currentMousePos = { 
				(currentMousePos.x - nowScreenXY.x)*zoomLevel,
				(currentMousePos.y - nowScreenXY.y)*zoomLevel};

			puts("drawStart");
			printf("MouseX : %d, MouseY : %d\n", currentMousePos.x, currentMousePos.y);
			(*ritor)->setPosition(_currentMousePos, _currentMousePos, isDrawing = true);

			isMoving = false;
			oldX = _currentMousePos.x;
			oldY = _currentMousePos.y;
			selectIndex = -1;
		}
		else {
			POINT currentMousePos = { LOWORD(lParam), HIWORD(lParam) };
	...
	case WM_LBUTTONUP: {
		...
		if (isDrawing) {
			...
			printf("MouseX : %d, MouseY : %d\n", currentMousePos.x, currentMousePos.y);
			(*itor)->setPosition({ oldX, oldY }, _currentMousePos, false);
			...
			POINT vMove = { _currentMousePos.x - oldX, _currentMousePos.y - oldY }; //이동량(벡터)
			printf("vMove.x : %d, vMove.y : %d\n", vMove.x, vMove.y);
			POINT selectElementP1 = (*itor)->getPoint1();
			POINT selectElementP2 = (*itor)->getPoint2();
			POINT movedP1 = { 
				selectElementP1.x + (vMove.x*zoomLevel), 
				selectElementP1.y + (vMove.y*zoomLevel) };
			POINT movedP2 = { 
				selectElementP2.x + (vMove.x*zoomLevel), 
				selectElementP2.y + (vMove.y*zoomLevel) };
			...
			case 1: {
				...
				POINT vMove = { _currentMousePos.x - oldX, _currentMousePos.y - oldY };
				POINT selectElementP1 = (*itor)->getPoint1();
				POINT selectElementP2 = (*itor)->getPoint2();
				POINT movedP1 = { 
					selectElementP1.x + (vMove.x*zoomLevel), 
					selectElementP1.y + (vMove.y*zoomLevel)};
				(*itor)->setPosition(movedP1, selectElementP2, false);
				break;
			}
			...
			case 2: {
				...
				POINT movedP2 = { 
					selectElementP2.x + (vMove.x*zoomLevel), 
					selectElementP2.y + (vMove.y*zoomLevel)};
				(*itor)->setPosition(selectElementP1, movedP2, false);
				break;
			}
			}
	...
	case WM_MOUSEMOVE: {
		if (isDrawing) {
			...
			POINT _currentMousePos = {
				(currentMousePos.x - nowScreenXY.x)*zoomLevel,
				(currentMousePos.y - nowScreenXY.y)*zoomLevel };
			(*itor)->setPosition({ oldX,oldY }, _currentMousePos, isDrawing);
			...
			case 1: {
				...
				POINT movedP1 = { 
					selectElementP1.x + (vMove.x*zoomLevel), 
					selectElementP1.y + (vMove.y*zoomLevel)};
				(*itor)->setPosition(movedP1, selectElementP2, true);
			...
			case 2: {
				...
				POINT movedP2 = { 
					selectElementP2.x + (vMove.x*zoomLevel), 
					selectElementP2.y + (vMove.y*zoomLevel)};
				(*itor)->setPosition(selectElementP1, movedP2, true);
