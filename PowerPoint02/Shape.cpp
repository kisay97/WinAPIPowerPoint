...
bool Shape::isCanResize1(POINT mousePosition, POINT nowScreenXY, double zoomLevel) {
	...
	bool x = (mousePosition.x >= p1.x - 5 && mousePosition.x <= p1.x + 5);
	bool y = (mousePosition.y >= p1.y - 5 && mousePosition.y <= p1.y + 5);
	if (x && y) {
		return true;
	}
	else {
...
bool Shape::isCanResize2(POINT mousePosition, POINT nowScreenXY, double zoomLevel) {
	...
	bool x = (mousePosition.x >= p2.x - 5 && mousePosition.x <= p2.x + 5);
	bool y = (mousePosition.y >= p2.y - 5 && mousePosition.y <= p2.y + 5);
	if (x && y) {
		return true;
	}
	else {
